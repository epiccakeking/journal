...
class SettingsModal(Gtk.Window):
    ...
    def __init__(self, parent):
        ...
        self.word_cloud_exclusions_button.connect(
            "clicked",
            lambda *_: self.change_page(
                SetManager(
                    self.word_cloud_exclusions,
                    lambda w: self.set_word_cloud_exclusions(w.get_list()),
                )
            ),
        )
        self.present()
